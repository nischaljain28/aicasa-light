...
module.exports = function (homebridge) {
    ...
    homebridge.registerAccessory("homebridge-aicasa-light", "aicasa-light");
};
...
function HTTP_LIGHTBULB(log, config) {
    ...
    var authCode = stringify(config.authCode);
    var pin = stringify(config.pin);
...
HTTP_LIGHTBULB.prototype = {
    ...
    getServices: function () {
        ...
        informationService
            .setCharacteristic(Characteristic.Manufacturer, "AiCasa Smart Homes")
            .setCharacteristic(Characteristic.Model, "AiCasa Light")
            .setCharacteristic(Characteristic.SerialNumber, "ASH.LB.01")
    ...
    getPowerState: function (callback) {
        ...
        var getSatus = "https://blynk.cloud/external/api/get?token="+authCode+"&"+pin;

        http.httpRequest(getSatus, (error, response, body) => {
            if (error) {
    ...
    setPowerState: function (on, callback) {
        ...
        var setOn = "https://blynk.cloud/external/api/update?token="+authCode+"&"+pin+"=1";
        var setOff = "https://blynk.cloud/external/api/update?token="+authCode+"&"+pin+"=0";
        ...
        if (!this.power.isMqtt) {
            const urlObject = on ? setOn : setOff;
